...
function drawCards() {
  ...
  $.each(Object.keys(cardsInPlay["cards"]), function (i, cardName) {
    ...
    if (isHost) {
      ...
      $("#cardPreviewPane").append(
        '<div class="col-sm-4 card"><img src="' +
          src +
          '" class="cardPreviewImg"><a class="unselectable">' +
          cardName +
  ...
  if (isHost) {
    ...
    console.log(
      "cards: " +
        Object.keys(cardsInPlay["cards"]).length +
        ", players: " +
        players.length
    );
    if (Object.keys(cardsInPlay["cards"]).length < players.length) {
      $("#numCards").text(
        "Still need " +
          (
            players.length - Object.keys(cardsInPlay["cards"]).length
          ).toString() +
          " card(s)."
      );
      ...
      $("#numCards").text(
        "You've selected " +
          (
            Object.keys(cardsInPlay["cards"]).length - players.length
          ).toString() +
          " too many card(s)!"
      );
      socket.emit("cards balanced", false);
